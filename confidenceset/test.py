...
def fdr_error_check(n_subj, img_dim, c, noise_set, mu_set, method,
                    var=1, alpha=0.05, tail="two"):
    data_dim = np.array((n_subj,) + img_dim)
    noise = get_noise(noise_set, data_dim) * var
    mu = get_mu(mu_set, data_dim)
    ...
    Ac = mu >= c
    AcC = 1 - Ac
    lower, upper, Achat, all_sets, n_rej = fdr_cope(data, method=method, threshold=c, alpha=0.05, tail=tail)
